...
let importErrors = []
// There are two ways the prettier config can be defined:
try {
	const require = createRequire(import.meta.url)
	// The old way was a json file:
	...
	importErrors.push(e0)

	// Second try:
	try {
		// The new way is a .mjs file:
		PrettierConf = await import('@sofie-automation/code-standard-preset/prettier.config.mjs')
	} catch (e1) {
		importErrors.push(e1)
	}
}
if (!PrettierConf) {
	importErrors.forEach(console.error)
