...
import { BlueprintEntrypoints, BlueprintBundles } from './blueprint-map.mjs'
// import './package.json' with { type: json}// experimental import
...
const pkgIntegration = (
	await import('@sofie-automation/blueprints-integration/package.json', {
		assert: { type: 'json' },
	})
).default

const production = process.env.NODE_ENV === 'production' // TODO - check this
...
let sources = BlueprintEntrypoints
if (process.env.bundle && process.env.bundle !== 'all') {
	const bundle = BlueprintBundles[process.env.bundle]
	if (bundle) {
	...
	for (const name of bundle) {
		if (BlueprintEntrypoints[name]) {
			sources[name] = BlueprintEntrypoints[name]
		} else {
...
export default async function (cliOptions) {
	const server = cliOptions['config-server'] || ''
