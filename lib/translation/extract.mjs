...
function defaultSourceGlobs(entryPointName) {
	return [`src/${entryPointName}/**/*.ts`, `src/${entryPointName}/**/*.json`]
}
...
async function clearNamespacePoFiles(entryPointName, locales) {
	for (const language of locales) {
		const poPath = path.join('locales', language, `${entryPointName}.po`)
...
async function extractFromSources(entryPointName, sourceGlobs, parserOptions) {
	...
	await pipeline(
		vfs.src(sourceGlobs, { allowEmpty: true }),
...
export async function extractTranslations(blueprintMapOrEntrypoints) {
	...
	const useFeatureBasedDefaults = isBlueprintMap && blueprintMapOrEntrypoints.BlueprintTranslationSources !== undefined
	...
	for (const [entryPointName, sourcePath] of Object.entries(entrypoints)) {
		const sourceGlobs = useFeatureBasedDefaults
			? scanPaths[entryPointName] ?? defaultSourceGlobs(entryPointName)
			: resolveSourceGlobs(entryPointName, sourcePath, scanPaths)

		await extractFromSources(entryPointName, sourceGlobs, parserOptions)
	}
}
