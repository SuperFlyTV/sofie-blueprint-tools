...
export async function extractTranslations(BlueprintEntrypoints) {
	for (const [entryPointName, sourcePath] of Object.entries(BlueprintEntrypoints)) {
		const start = Date.now()
		const entryPointRoot = path.parse(sourcePath).dir
		console.info(`\nExtracting keys from ${entryPointRoot} ...`)
