...
class SkipUnchangedPoTransform extends stream.Transform {
	constructor() {
		super({ objectMode: true })
	}

	_transform(file, encoding, callback) {
		const poPath = poOutputPath(file)
...
async function clearNamespacePoFiles(entryPointName, locales) {
	for (const language of locales) {
		...
		const existing = await readFile(poPath, 'utf-8').catch(() => null)
		if (existing === poContent) {
			continue
		}
